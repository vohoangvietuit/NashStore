...
function Home () {
  ...
  const triggerChange = () => {
    if (searchKey && searchKey.length > 0) {
      ...
      let url = `/api/products/suggest-product?search=${searchKey}`;
      if (categoryKey && categoryKey !== 'null' && categoryKey !== null) {
  ...
  const handleClick = value => {
    ...
    history.push({
      pathname: '/home',
      search: `?search=${value}&category=${categoryKey}&page=1`
    });
  ...
  const checkExistInCart = (productId) => {
    if (cart.carts.findIndex(item => (item.id || item._id) === productId) > -1) {
      return true;
    }
    return false;
  }
    ...
    const productContent = dataProducts.length ? (
      dataProducts.map((product, index) => (
        <Item
          ...
          disabledCart={checkExistInCart(product.id || product._id)}
        />
      ))
    ) : (
